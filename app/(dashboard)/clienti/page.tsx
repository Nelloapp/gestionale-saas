      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        ...
        {filtrati.map(c => (
          <div key={c.id} style={{ background: 'white', borderRadius: '12px', padding: '16px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
              <div style={{ flex: 1 }}>
                ...
                <p style={{ color: '#64748b', fontSize: '12px', marginTop: '2px' }}>
                  {c.tipo === 'azienda' ? '🏢' : c.tipo === 'privato' ? '👤' : '🏛️'} {c.tipo} · {c.citta}{c.provincia ? ` (${c.provincia})` : ''}
                </p>
              </div>
            ...
            {c.sconto > 0 && <p style={{ color: '#f59e0b', fontSize: '12px', marginBottom: '8px', fontWeight: '600' }}>🏷️ Sconto: {c.sconto}% · Fido: €{(c.fido || 0).toLocaleString('it-IT')}</p>}
          ...
          <div style={{ background: 'white', borderRadius: '16px 16px 0 0', padding: '24px', width: '100%', maxHeight: '92vh', overflowY: 'auto' }}>
            ...
            <div style={{ marginBottom: '14px' }}>
              ...
              <div style={{ background: '#f8fafc', borderRadius: '10px', padding: '12px', display: 'grid', gap: '8px' }}>
                {[['P.IVA', schedaAperta.piva], ['Cod. Fiscale', schedaAperta.cf], ['Cod. SDI', schedaAperta.sdi], ['PEC', schedaAperta.pec],
                  ['Indirizzo', `${schedaAperta.indirizzo || ''}, ${schedaAperta.cap || ''} ${schedaAperta.citta || ''}`], ['Paese', schedaAperta.paese],
                ].filter(([, v]) => v).map(([k, v]) => (
            ...
            <div style={{ marginBottom: '14px' }}>
              ...
              <div style={{ background: '#f8fafc', borderRadius: '10px', padding: '12px', display: 'grid', gap: '8px' }}>
                {[['Listino', listinoLabel[schedaAperta.listino] || schedaAperta.listino],
                  ...
                  ['Agente', schedaAperta.agente || '—'], ['Categoria', schedaAperta.categoria],
                ].map(([k, v]) => (
                  <div key={k as string} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
                    <span style={{ color: '#64748b' }}>{k}</span>
